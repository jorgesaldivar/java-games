//***************** vv DOCUMENTING AND HELPFUL STUFF vv ********************
		...
		//Compiled in CMD Promt using "javac TicTacToe.java" 
		...
		//Run in cmd promt using "java TicTacToe"
...
import java.awt.event.*;	
   ...
   // class constants 	
      //String [] name={"TicTacToeConBotas.gif", "porky.gif","perroADieta.gif","Leon.gif","caballo.gif","caballoBlanco.gif"};
      ...
      private static final String LEGEND = "You are playing TicTacToe!\nprogrammed by \nJorge Andres Saldivar Hernandez"	
      + "\nID: 1033317";	
       ...
       public void initialize()
      {
         for(int r=0, i=0; r<m.length; r++)
            for (int c=0; c<m[0].length; c++,i++)
            {
               ...
               m[r][c].setIcon(new ImageIcon("yellow.jpg")); //background            	
            }
      }
   ...
   // actionPerformed(): run button action event handler	
      ...
      // get user's responses 	
            ...
            for(int c=0;c<m[r].length&&!find;c++)  
               if(e.getSource()==m[r][c]){
                  ...
                  if(m[r1][c1].getText().equals("")){
                     m[r1][c1].setIcon(new ImageIcon("x.jpg"));
                     m[r1][c1].setText("xx");
                     computerThrow=true;
                  }
               }
         ...
         if(won){
            JOptionPane.showMessageDialog(null,"You won!","TicTacToe",-1,new ImageIcon("winner.jpg"));
            for(int r=0;r<m.length;r++)
               for(int c=0;c<m[r].length;c++){
                  m[r][c].setText("");
                  m[r][c].setIcon(new ImageIcon("yellow.jpg"));}      
            computerThrow=false;
            won=false;
            draw=false;
            playerCount++;  
            ...
            for(int r=(int)(Math.random()*2);r<m.length;r++)
               for(int c=(int)(Math.random()*2);c<m[r].length;c++)
                  if(m[r][c].getText().equals("")&&!find){
                     ...
                     draw=false;
                     m[r][c].setText("0.o");
                     m[r][c].setIcon(new ImageIcon("o.jpg"));
         ...
         if(won){
            JOptionPane.showMessageDialog(null,"You Lose...","TicTacToe",-1,new ImageIcon("loser.jpg"));
            for(int r=0;r<m.length;r++)
               for(int c=0;c<m[r].length;c++){
                  m[r][c].setText("");
                  m[r][c].setIcon(new ImageIcon("yellow.jpg"));}
